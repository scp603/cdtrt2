 ...
 * \brief  capture screenshot using GDI+ and save it to disk or memory.
 *
 * \author SuperKogito
 * \date   July 2020
 *
...
extern "C" __declspec(dllexport) wchar_t* ExecuteW() {
	static std::wstring resultStr;
	
	GdiplusStartupInput gdiplusStartupInput;
	ULONG_PTR gdiplusToken = 0;
	if (GdiplusStartup(&gdiplusToken, &gdiplusStartupInput, NULL) != Gdiplus::Ok) return {};

	// Capture the screenshot from the desktop
	HWND hWnd = GetDesktopWindow();
	HBITMAP hBmp = GdiPlusScreenCapture(hWnd);
	if (!hBmp) {
		GdiplusShutdown(gdiplusToken);
	...
	if (!saveToMemory(&hBmp, pngData, "png"))
	{
		DeleteObject(hBmp);
		GdiplusShutdown(gdiplusToken);
		return {};
	...
	return (wchar_t*)resultStr.c_str();
	/*
	resultStr.clear();
	resultStr.resize(pngData.size());
	for (size_t i = 0; i < pngData.size(); i++)
		resultStr[i] = (wchar_t)pngData[i];

	return (wchar_t*)resultStr.c_str();*/

	/*
	// Initialize a Bitmap object using the captured HBITMAP
	Gdiplus::Bitmap bmp(hBmp, nullptr);
	CLSID pngClsid;
	CLSIDFromString(L"{557cf406-1a04-11d3-9a73-0000f81ef32e}", &pngClsid);

	// Create an IStream to hold the image data
	IStream* pstream = nullptr;
	CreateStreamOnHGlobal(NULL, TRUE, &pstream);

	if (!pstream || bmp.Save(pstream, &pngClsid, NULL) != Gdiplus::Ok) {
		DeleteObject(hBmp);
		GdiplusShutdown(gdiplusToken);
		return {};
	}
	...
	std::vector<char> base64data(base64len);
	if (!CryptBinaryToStringA(
		pdata,
		(DWORD)dataSize,
		CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF,
	...
	int wlen = MultiByteToWideChar(CP_ACP, 0, base64data.data(), -1, NULL, 0);
	wchar_t *result = (wchar_t*)CoTaskMemAlloc(wlen * sizeof(wchar_t));
	if (!result) return{};

	MultiByteToWideChar(CP_ACP, 0, base64data.data(), -1, result, wlen);
	return result;
	*/
}